...
//  SpriteTestRect.cpp
//
//  Rectangular sprite collision demo test, with audio.
//
...
int main( int argc, char** argv )
{
    ...
    collisions.reserve( NUM_RECTS * NUM_RECTS );
    ...
    for( int i = 0; i < NUM_RECTS; i++ )
    {
        int size_w;
        {
            int temp = rand() % 100;
            if( temp <= 50 ) { size_w = 4; }
            else if( temp <= 75 ) { size_w = 3; }
            else if( temp <= 87 ) { size_w = 2; }
            else { size_w = 1; }
        }
        ...
        int size_h = std::max( 1, size_w + rand() % 2 );
    ...
    do
    {
        ...
        // Action collisions
        {
            ...
            for( MCK::CollisionEvent coll : collisions )
            {
                ...
                {
                    // Convert SpritePos shared pointers
                    // to SpriteCollisionRect regular pointers
                    MCK::SpriteCollisionRect* coll_rect_A;
                    MCK::SpriteCollisionRect* coll_rect_B;
                    try
                    {
                        coll_rect_A
                            = dynamic_cast<MCK::SpriteCollisionRect*>(
                                &*coll.sprite_A
                            );
                        coll_rect_B
                            = dynamic_cast<MCK::SpriteCollisionRect*>(
                    ...
                    {
                        throw( std::runtime_error(
                            std::string( "Failed to recast sprites as SpriteCollisionRect, error :" )
                            + e.what() ) );
                    }
                    ...
                    MCK::SpriteMotionConstVel::elastic_collision_rect(
                        ...
                        *static_cast<MCK::GEO::Rectangle<float>*>(coll_rect_A ),
                        *static_cast<MCK::GEO::Rectangle<float>*>(coll_rect_B ),
